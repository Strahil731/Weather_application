...
export function updateScreenReaderConfirmation(message) {
    document.getElementById("confirmation").textContent = message;
}

export function updateDisplay(weatherJson, locationObj) {
    fadeDisplay();
    clearDisplay();
    ...
    // current conditions
    const ccArray = cretaeCurrentConditionsDivs(weatherJson, locationObj.getUnit());
...
function clearDisplay() {
    ...
    const sixDayForecast = document.getElementById("dayliForecast_contents");
...
function setBGImage(weatherClass) {
...
function buildScreenReaderWeather(weatherJson, locationObj) {
...
function cretaeCurrentConditionsDivs(weatherObj, unit) {
...
function createMainImgDiv(icon, altText) {
...
function createElem(elemType, divClassName, divText, unit) {
...
function translateIconToFontAwesome(icon) {
...
function displayCurrentConditions(currentConditionsArray) {
...
function displaySixDayForecast(weatherJson) {
    for (let i = 1; i <= 6; i++) {
...
function getDayAbbreviation(data) {
...
function createDailyForecastIcon(icon, altText) {
...
function displayDailyForecast(dfArray) {
    const dayDiv = createElem("div", "forecastDay");
    dfArray.forEach((el) => {
        dayDiv.appendChild(el);
    });

    const dailyForecastContainer = document.getElementById("dayliForecast_contents");
